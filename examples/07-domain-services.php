...
foreach ($forwards as $fwd) {
    echo "  {$fwd->fullUrl($domainName)} -> {$fwd->location}\n";
}
...
// Glue Records
$glueRecords = $domain->glueRecords()->all();
echo "\nGlue records: " . count($glueRecords) . "\n";
foreach ($glueRecords as $glue) {
    echo "  {$glue->fullHostname($domainName)} -> " . implode(', ', $glue->ips) . "\n";
}

// $domain->glueRecords()->create('ns1', '192.0.2.1', '192.0.2.2');
// $domain->glueRecords()->update('ns1', '192.0.2.10');
// $domain->glueRecords()->delete('ns1');
