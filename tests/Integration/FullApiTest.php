...
test('domains: list all domains in account', function () use ($client, $domain): void {
    $domainCollection = $client->domains()->all();

    expect($domainCollection)->toBeInstanceOf(DomainCollection::class)
        ->and($domainCollection->count())->toBeGreaterThan(0)
...
test('domains: allPages generator yields domains', function () use ($client, $domain): void {
    ...
    foreach ($client->domains()->allPages() as $domainDto) {
        expect($domainDto)->toBeInstanceOf(Domain::class);
...
test('dns: batch operations', function () use ($domainResource): void {
    ...
    foreach ($results as $result) {
        expect($result->success)->toBeTrue()
            ->and($result->operation)->toBe('create');
...
test('glue records: create, update, delete lifecycle', function () use ($domainResource, $domain): void {
    $glue = $domainResource->glue();
    $subdomain = 'testns1';
    ...
    try {
        ...
        $found = $all->find("{$subdomain}.{$domain}");
        expect($found)->not->toBeNull()
        ...
        $updated = $afterUpdate->find("{$subdomain}.{$domain}");
    ...
    // Verify deletion
    $glueRecordCollection = $glue->all();
    $deleted = $glueRecordCollection->find("{$subdomain}.{$domain}");
    expect($deleted)->toBeNull();
})->group('integration');
...
test('auto-renew: disable and re-enable', function () use ($domainResource): void {
    ...
    try {
        ...
        expect($autoRenew->disable())->toBeBool()
            ->and($autoRenew->enable())->toBeBool();
    } finally {
