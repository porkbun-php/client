...
test('fromArray creates collection from array data', function (): void {
    ...
    expect($glueRecordCollection)->toHaveCount(2)
        ->and($glueRecordCollection->items())->toHaveCount(2)
        ->and($glueRecordCollection->items()[0])->toBeInstanceOf(GlueRecord::class)
        ->and($glueRecordCollection->items()[0]->host)->toBe('ns1');
});
...
test('find is case-insensitive', function (): void {
    ...
    expect($glueRecordCollection->find('NS1'))->toBeInstanceOf(GlueRecord::class)
        ->and($glueRecordCollection->find('Ns1'))->toBeInstanceOf(GlueRecord::class)
        ->and($glueRecordCollection->has('NS1'))->toBeTrue();
});
...
test('has checks for record existence by host', function (): void {
    $glueRecordCollection = GlueRecordCollection::fromArray([
        ['host' => 'ns1', 'ips' => ['192.0.2.1']],
        ['host' => 'ns2', 'ips' => ['192.0.2.2']],
    ]);

    expect($glueRecordCollection->has('ns1'))->toBeTrue()
        ->and($glueRecordCollection->has('ns2'))->toBeTrue()
        ->and($glueRecordCollection->has('ns99'))->toBeFalse();
});
