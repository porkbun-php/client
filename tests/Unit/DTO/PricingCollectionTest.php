...
test('items returns indexed array', function (): void {
    ...
    $all = $pricingCollection->items();

    expect($all)->toBeArray()
        ->and(array_keys($all))->toBe([0, 1])
        ->and($all[0])->toBeInstanceOf(PricingItem::class);
});

test('allByTld returns associative array', function (): void {
    $pricingCollection = PricingCollection::fromArray([
        'com' => ['registration' => '8.99', 'renewal' => '10.99'],
    ]);

    $byTld = $pricingCollection->allByTld();

    expect($byTld)->toBeArray()
        ->and(array_keys($byTld))->toBe(['com'])
        ->and($byTld['com'])->toBeInstanceOf(PricingItem::class);
});
