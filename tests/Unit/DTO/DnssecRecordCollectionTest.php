...
test('fromArray creates collection from array data', function (): void {
    ...
    expect($dnssecRecordCollection)->toHaveCount(2)
        ->and($dnssecRecordCollection->items())->toHaveCount(2)
        ->and($dnssecRecordCollection->items()[0])->toBeInstanceOf(DnssecRecord::class)
        ->and($dnssecRecordCollection->items()[0]->keyTag)->toBe(12345)
        ->and($dnssecRecordCollection->items()[1]->keyTag)->toBe(67890);
});
...
test('toArray and jsonSerialize return same data', function (): void {
    ...
    expect($dnssecRecordCollection->toArray())->toBe($dnssecRecordCollection->jsonSerialize())
        ...
        ->and($dnssecRecordCollection->toArray()[0])->toHaveKey('algorithm');
});

test('has checks for record existence by keyTag', function (): void {
    $dnssecRecordCollection = DnssecRecordCollection::fromArray([
        ['keyTag' => '111', 'algorithm' => '13', 'digestType' => '2', 'digest' => 'aaa'],
        ['keyTag' => '222', 'algorithm' => '8', 'digestType' => '1', 'digest' => 'bbb'],
    ]);

    expect($dnssecRecordCollection->has(111))->toBeTrue()
        ->and($dnssecRecordCollection->has(222))->toBeTrue()
        ->and($dnssecRecordCollection->has(999))->toBeFalse();
});
