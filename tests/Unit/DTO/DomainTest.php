...
test('it creates domain from array', function (): void {
    ...
    expect($domainDTO->domain)->toBe('example.com')
        ...
        ->and($domainDTO->isExternal)->toBeFalse();
});
...
test('it parses boolean flags strictly', function (): void {
    ...
    expect($domain->securityLock)->toBeTrue()
        ...
        ->and($domain->isExternal)->toBeTrue();
    ...
    expect($falsy->securityLock)->toBeFalse()
        ...
        ->and($falsy->isExternal)->toBeFalse();
    ...
    expect($edgeCases->securityLock)->toBeFalse()
        ...
        ->and($edgeCases->isExternal)->toBeFalse();
});
...
test('labels are always 0-indexed even with filtered gaps', function (): void {
    ...
    expect(array_keys($labels))->toBe([0, 1])
        ->and($labels[0]->title)->toBe('Production')
        ->and($labels[1]->title)->toBe('Critical');
});
...
test('toArray serializes all fields correctly', function (): void {
    ...
    expect($array['domain'])->toBe('example.com')
        ...
        ->and($array['createDate'])->toContain('2020-01-15T10:30:00')
        ->and($array['expireDate'])->toContain('2025-01-15T10:30:00')
        ->and($array['securityLock'])->toBeTrue()
        ->and($array['whoisPrivacy'])->toBeTrue()
        ->and($array['autoRenew'])->toBeFalse()
        ->and($array['isExternal'])->toBeTrue()
