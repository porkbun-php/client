...
it('converts to array with canonical field names', function (): void {
...
it('round-trips through fromArray and toArray', function (): void {
    ...
    $roundTripped = DnssecRecord::fromArray($original->toArray());

    expect($roundTripped->keyTag)->toBe($original->keyTag)
        ->and($roundTripped->algorithm)->toBe($original->algorithm)
        ->and($roundTripped->digestType)->toBe($original->digestType)
        ->and($roundTripped->digest)->toBe($original->digest)
        ->and($roundTripped->maxSigLife)->toBe($original->maxSigLife)
        ->and($roundTripped->flags)->toBe($original->flags)
        ->and($roundTripped->protocol)->toBe($original->protocol)
        ->and($roundTripped->publicKey)->toBe($original->publicKey);
});
...
it('handles optional fields correctly', function (): void {
    ...
    expect($array)->not->toHaveKey('maxSigLife')
        ->and($array)->not->toHaveKey('flags')
        ->and($array)->not->toHaveKey('protocol')
        ->and($array)->not->toHaveKey('publicKey');
});
