...
test('network exception accepts and stores a response', function (): void {
    $factory = new Psr17Factory();
    $request = $factory->createRequest('POST', 'https://api.example.com/test');
    $response = $factory->createResponse(502);

    $exception = new NetworkException('Bad gateway', 502, null, $request, $response);
