...
test('dns api captures cloudflare status on collection', function (): void {
    $mockClient = createMockHttpClient([
        [
            'status' => 'SUCCESS',
            'cloudflare' => 'enabled',
    ...
    expect($dnsRecordCollection)->toBeInstanceOf(DnsRecordCollection::class)
        ->and($dnsRecordCollection->cloudflare)->toBe('enabled')
...
test('dns api handles missing cloudflare status', function (): void {
    ...
    expect($dnsRecordCollection)->toBeInstanceOf(DnsRecordCollection::class)
        ->and($dnsRecordCollection->cloudflare)->toBeNull()
...
test('dns api findByType captures cloudflare status on collection', function (): void {
    $mockClient = createMockHttpClient([
        [
            'status' => 'SUCCESS',
            'cloudflare' => 'enabled',
...
test('dns api can update record', function (): void {
    ...
    $result = $dns->update(123, 'A', 'www', '192.0.2.2');
...
test('dns api can update record with optional params', function (): void {
    ...
    $result = $dns->update(123, 'MX', 'mail', 'mail.example.com', ttl: 3600, priority: 10, notes: 'Updated record');
...
test('dns api update sends empty string notes to clear them', function (): void {
    ...
    $result = $dns->update(123, 'A', 'www', '192.0.2.1', notes: '');
...
test('dns api update omits notes when null', function (): void {
    ...
    $result = $dns->update(123, 'A', 'www', '192.0.2.1');
...
test('dns api can update record from builder', function (): void {
    ...
    $result = $dns->updateFromBuilder(789, $dnsRecordBuilder);
...
test('dns api can update by type and name', function (): void {
    ...
    $result = $dns->updateByType('A', 'www', '192.0.2.3');
...
test('dns api updateByType requires content parameter', function (): void {
    $mockClient = createMockHttpClient([]);

    $httpClient = createHttpClient($mockClient, 'pk1_key', 'sk1_secret');
    $dns = new Dns(createMockContext($httpClient), 'example.com');

    expect(fn (): OperationResult => $dns->updateByType('A', 'www')) // @phpstan-ignore-line
